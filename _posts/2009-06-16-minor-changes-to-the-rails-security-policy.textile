...
---
After reviewing the feedback on the two recent security announcements we've made a few minor changes to the "Ruby on Rails security policy":http://rubyonrails.org/security.  
...
The second change is to more clearly outline the announcement policy for rails vulnerabilities.  In short, we notify vendor-sec ahead of the public notification to allow time for people distributing rails to prepare packages for their distributions.  Then when the time has come for public notification an email is sent to the "security announcement list":http://groups.google.com/group/rubyonrails-security.  Finally the announcement is posted to this blog.
...
If you have any comments on the security policy, please send them via email to <a href="mailto:security@rubyonrails.org">security@rubyonrails.org</a>.